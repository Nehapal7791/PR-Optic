...
Implements the AIProvider interface using Google's Gemini 2.0 Flash model.
...
class GeminiProvider(AIProvider):
    """Google Gemini AI provider for code review."""
    ...
    def __init__(self):
        ...
        logger.info(f"GeminiProvider initialized with model=gemini-pro, max_tokens={self._max_tokens}")
    ...
    def _extract_json_from_response(self, text: str) -> dict:
        ...
        # But we'll keep the fallback logic for safety
        try:
    ...
    async def triage_diff(
        ...
        logger.info(f"Triaging diff with {len(files)} files using Gemini")
...
Only include categories that have genuine concerns. If the PR looks good, return empty categories array."""
        ...
        try:
            response = self.model.generate_content(triage_prompt)
    ...
    async def review_pull_request(
        ...
        logger.info(f"Reviewing PR with {len(categories)} categories using Gemini")
