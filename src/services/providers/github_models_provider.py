...
Uses GitHub's AI models marketplace (OpenAI-compatible API).
Free tier: 10 requests/minute (limited but free).
"""
...
class GitHubModelsProvider(AIProvider):
    """GitHub Models AI provider for code review."""
    ...
    def __init__(self):
        ...
        self.model_name = "gpt-4o"  # Can also use: claude-3.5-sonnet, llama-3.1-405b
        self._max_tokens = 4000
        logger.info(f"GitHubModelsProvider initialized with model={self.model_name}, max_tokens={self._max_tokens}")
    ...
    async def triage_diff(
        ...
        logger.info(f"Triaging diff with {len(files)} files using GitHub Models")
...
Only include categories that have genuine concerns. If the PR looks good, return empty categories array."""
    ...
    async def review_pull_request(
        ...
        logger.info(f"Reviewing PR with {len(categories)} categories using GitHub Models")
